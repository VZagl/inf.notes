...
**Оформление задач:** каждая задача — заголовок `### [ ] Название (`task-id`)`; метаданные — плоский список `- **Ключ:** значение` под заголовком (не `- [ ]` в списке). Раздел **Идеи** — обычные пункты `-`. Подробнее — «Добавление новой задачи в backlog» ниже.
...
1. **Дата создания** — первая строка метаданных под заголовком задачи: `- **Создано:** YYYY-MM-DD`
...
**Формат задачи** (задача — **не** пункт `- [ ]` списка, а заголовок `###`):
...
- **Создано:** YYYY-MM-DD
- **Цель:** …
- **Источник:** [путь и step-id, например docs/project/implementation-plan.md (`step-id`)] или «—»
- **Зависит от:** …
```

- Заголовок задачи: `### [ ]` (активная) или `### [x]` (если когда-либо фиксируете выполнение до `/close-task` — редко; обычно задачу удаляют из backlog).
- Метаданные — **плоский** маркированный список `- **Ключ:** значение` сразу под заголовком; **не** вкладывать метаданные в `- [ ]` родительский пункт.
- Вложенные списки допустимы только для подпунктов одного поля (например варианты A–D под `- **Варианты:**`).
- Раздел **Идеи** — по-прежнему обычные `-` без `###`.
- Пустые строки: одна между `###` и первым `- **…**`; между соседними пунктами одного списка метаданных пустых строк нет ([markdown-conventions.md](markdown-conventions.md)).
