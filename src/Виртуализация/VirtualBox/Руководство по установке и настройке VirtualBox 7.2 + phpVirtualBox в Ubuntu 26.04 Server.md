...
> **Не включайте оба способа** для одной и той же ВМ — выберите либо §7.1, либо §7.2. Если ошибочно включили литерал `ИМЯ_ВАШЕЙ_ВМ`: `sudo systemctl disable --now 'vbox-vm@ИМЯ_ВАШЕЙ_ВМ.service'` и `sudo systemctl reset-failed 'vbox-vm@ИМЯ_ВАШЕЙ_ВМ.service'`.
