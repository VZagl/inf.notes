...
- [0. Время, часовой пояс и отключение автообновлений](#0-время-часовой-пояс-и-отключение-автообновлений)
  - [0.1. Часовой пояс Europe/Kyiv](#01-часовой-пояс-europekyiv)
  - [0.2. Синхронизация времени через интернет](#02-синхронизация-времени-через-интернет)
  - [0.3. Отключение автообновлений](#03-отключение-автообновлений)
...
- [7. Включение автозапуска](#7-включение-автозапуска)
  - [7.1. Через `vboxautostart` (если `VBoxAutostart --start` поднял ВМ)](#71-через-vboxautostart-если-vboxautostart---start-поднял-вм)
  - [7.2. Через `vbox-vm@…` (если было `No virtual machines found` или §5.2)](#72-через-vbox-vm-если-было-no-virtual-machines-found-или-52)
...
- [10. Сквозной (raw) диск и режим Writethrough](#10-сквозной-raw-диск-и-режим-writethrough)
  - [10.1. Определите устройство и каталог для VMDK](#101-определите-устройство-и-каталог-для-vmdk)
  - [10.2. Права доступа к физическому диску](#102-права-доступа-к-физическому-диску)
  - [10.2.1. Изоляция raw-диска от хоста (udev, fstab)](#1021-изоляция-raw-диска-от-хоста-udev-fstab)
  - [10.3. Создайте raw-VMDK](#103-создайте-raw-vmdk)
  - [10.4. Подключите диск к ВМ и включите Writethrough](#104-подключите-диск-к-вм-и-включите-writethrough)
  - [10.5. Устранение проблем](#105-устранение-проблем)
  - [10.6. ВМ в состоянии Aborted: вмешательство хоста в raw-диск](#106-вм-в-состоянии-aborted-вмешательство-хоста-в-raw-диск)
...
## 0. Время, часовой пояс и отключение автообновлений

Выполните этот раздел **до** [§1](#1-установка-virtualbox-и-подготовка-пользователя-vbox) — и на чистой установке, и на уже работающем сервере. Часовой пояс нужен, чтобы `journalctl` совпадал с местным временем. Автообновления apt на хосте с запущенными ВМ отключаются: `unattended-upgrade` обновляет библиотеки и перезапускает `vboxweb`, после чего VirtualBox снимает ВМ с питания. В phpVirtualBox это часто статус **`Aborted`**.

### 0.1. Часовой пояс Europe/Kyiv
...
Ожидаемо: `Time zone: Europe/Kyiv`. Зимой смещение `EET (+0200)`, летом `EEST (+0300)` — переключает systemd, вручную его не задают.

Журнал хранит метки в UTC, а `journalctl` показывает их в текущей зоне. После смены пояса событие, которое при `Etc/UTC` выглядело как `06:13`, отображается как `09:13`.

### 0.2. Синхронизация времени через интернет

Проверьте, что часы уже синхронизируются:
...
Нужны строки `System clock synchronized: yes` и `NTP service: active`, служба `systemd-timesyncd` — `active`. Если так и есть, пакеты времени не ставьте: `chrony` не нужен.

Если `NTP service: inactive` или служба не `active`:
...
Повторная проверка — те же `yes` и `active`. Синхронизация может занять до минуты.

### 0.3. Отключение автообновлений

Остановите таймеры ежедневного `apt update` и `unattended-upgrade`:
...
Оба таймера должны быть `disabled`. Пакет `unattended-upgrades` удалять не нужно: без таймеров он не запускается.

Запретите периодический запуск и в конфиге apt (файл перезапишите целиком):
...
> **Почему так:** при автообновлении библиотек (например `libxml2`) Ubuntu перезапускает службы, которые их держат, в том числе `vboxweb`. Остановка веб-службы разбирает сессию VirtualBox и выполняет `powerDown` у работающих ВМ за доли секунды — без ACPI shutdown гостя и без Save State. В `VBox.log` это `Console::powerDown()` с `InUninit=1`.

**Ручное обновление** при запущенных ВМ — только в окне обслуживания. `stop` unit из [§5.2](#52-альтернатива-unit-на-каждую-вм) делает Save State. Имя ВМ — из `sudo -u vbox vboxmanage list vms`. Несколько ВМ останавливайте и запускайте по очереди. Обновление ядра и пакетов `virtualbox*` — только после Save State всех ВМ.
...
| Часовой пояс §0.1   | `timedatectl \| grep 'Time zone'`                                                                                            | `Europe/Kyiv`                                                            |
| NTP §0.2            | `timedatectl` и `systemctl is-active systemd-timesyncd`                                                                      | `System clock synchronized: yes`, `NTP service: active`, служба `active` |
| Автообновления §0.3 | `systemctl is-enabled apt-daily.timer apt-daily-upgrade.timer`                                                               | оба `disabled`                                                           |
...
KERNEL=="ДИСК*", ENV{UDISKS_IGNORE}="1"
EOF
...
> **Важно:** после настройки raw-диска выполните [§10.2.1](#1021-изоляция-raw-диска-от-хоста-udev-fstab) — без изоляции хост может «перехватить» устройство и аварийно завершить ВМ ([§10.6](#106-вм-в-состоянии-aborted-вмешательство-хоста-в-raw-диск)).

### 10.2.1. Изоляция raw-диска от хоста (udev, fstab)

VirtualBox с raw-VMDK держит **эксклюзивный** низкоуровневый доступ к `/dev/ДИСК`. Если хост перечитывает таблицу разделов или монтирует раздел на этом диске, `VBoxHeadless` теряет дескриптор и ВМ переходит в `Aborted`. Настройте изоляцию **до** первого запуска ВМ с raw-диском.

**1. Правило udev** (дополнение к §10.2 — запрет автосканирования и монтирования через `udisks2`):

```bash
# в том же файле 99-vbox-rawdisk.rules должны быть обе строки:
# KERNEL=="ДИСК", GROUP="vboxusers", MODE="0660"
# KERNEL=="ДИСК*", ENV{UDISKS_IGNORE}="1"
...
**2. Проверьте `/etc/fstab` хоста** — не должно быть записей для `/dev/ДИСК`, `/dev/ДИСКN` или UUID разделов с этого диска:

```bash
grep -E 'ДИСК|/dev/sd' /etc/fstab
```

**3. Не монтируйте и не размечайте raw-диск на хосте** (`fdisk`, `parted`, `gparted`, `mount`) при **запущенных** ВМ, использующих этот диск. Любое изменение таблицы разделов — только при остановленных ВМ.

**4. Права доступа** — в этом мануале пользователь `vbox` получает доступ через группу `vboxusers` и udev-правило §10.2 (режим `0660`). Добавлять `vbox` в системную группу `disk` **не требуется**, если §10.2 выполнен.
...
### 10.6. ВМ в состоянии Aborted: вмешательство хоста в raw-диск

**Симптомы:** утром или после длительного простоя ВМ в phpVirtualBox или `VBoxManage list vms` — статус **`Aborted`**. Процесс `VBoxHeadless` завершился аварийно (`SIGABRT`); в `VBox.log` часто **нет** записи о причине — падение настолько быстрое, что лог не успевает дописаться.

> **Другая причина, не raw-диск:** автообновление apt перезапускает `vboxweb` при работающих ВМ. В `VBox.log` сессия заканчивается `powerDown` и `InUninit=1`, файл лога записан в ту же секунду, что `Stopping vboxweb`. Отключение автообновлений — [§0.3](#03-отключение-автообновлений). Ниже в этом подразделе — только вмешательство хоста в блочное устройство.

**Диагностика:**

```bash
# события ядра по диску (подставьте имя устройства)
sudo dmesg -T | grep -E 'ДИСК|sd[a-z]:'

# журнал systemd unit ВМ (если используется §5.2)
sudo journalctl -u 'vbox-vm@*' --since "yesterday" --no-pager

# состояние устройства и разделов
lsblk -o NAME,SIZE,TYPE,MOUNTPOINT /dev/ДИСК
ls -l /dev/ДИСК /dev/ДИСК*
```

Если в `dmesg` **нет** ошибок I/O, SMART или «отвала» контроллера — аппаратная часть, как правило, исправна. Ищите признак **вмешательства хоста** в блочное устройство.

**Причина:**

1. До события ядро видело `/dev/ДИСК` как «сырой» диск или с неизменной таблицей разделов.
2. В определённый момент хост **перечитал таблицу разделов** или смонтировал раздел — типичные источники:
   - `udev` / `partx` после изменения разметки (`fdisk`, `parted`, `gparted`);
   - `udisks2` или другой менеджер монтирования;
   - появление или изменение записи в `/etc/fstab` и последующий mount.
3. Ядро отправляет событие `change` на блочное устройство; на короткое время перехватывается дескриптор `/dev/ДИСК`.
4. VirtualBox с raw-VMDK теряет открытый файл устройства → ассерт в `VBoxHeadless` → статус ВМ **`Aborted`**.

**Признак в `dmesg`:** появление раздела на ранее «сыром» диске вне запланированного обслуживания, например:

```text
[Пт июл 31 06:52:16 2026]  sda: sda1
```

Строка означает, что ядро **заново обнаружило** раздел `sda1` на диске `sda` — пересканирование таблицы разделов.

**Что делать:**

1. Выполните [§10.2.1](#1021-изоляция-raw-диска-от-хоста-udev-fstab) (udev `UDISKS_IGNORE`, проверка `fstab`, запрет монтирования на хосте).
2. Убедитесь, что разделы raw-диска **не** смонтированы на хосте: `findmnt /dev/ДИСК*`.
3. Запустите ВМ заново (`VBoxManage startvm …` или phpVirtualBox). Save State / snapshot для raw-диска в режиме Writethrough не восстанавливают согласованность данных на физическом носителе — при работе СУБД на raw-диске проверьте целостность гостя.
4. Если разметку на raw-диске меняли **намеренно** — делайте это **только при полностью остановленных** ВМ, использующих этот диск.

---

Готово. Сервер развёрнут с нуля: часовой пояс `Europe/Kyiv`, время синхронизируется через NTP, автообновления apt выключены ([§0](#0-время-часовой-пояс-и-отключение-автообновлений)); VirtualBox и Docker установлены, phpVirtualBox доступен по HTTP, ВМ автозапускаются и сохраняют состояние при выключении — с таймаутами, заданными только для служб VirtualBox. Добавление и вывод ВМ — [§9](#9-добавление-отключение-и-удаление-вм); сквозной диск — [§10](#10-сквозной-raw-диск-и-режим-writethrough) (изоляция raw-диска — [§10.2.1](#1021-изоляция-raw-диска-от-хоста-udev-fstab); падение в `Aborted` из-за диска — [§10.6](#106-вм-в-состоянии-aborted-вмешательство-хоста-в-raw-диск), из-за автообновления — [§0.3](#03-отключение-автообновлений)); при изменении набора ВМ пересчитайте `TimeoutStopSec` по таблице в §6.
