...
!!!NOTE При первом использовании `--tunnel` может потребоваться установка дополнительных зависимостей. Следуйте инструкциям в терминале.

!!!TIP Если у вас возникают проблемы с подключением, попробуйте перезапустить туннель командой `expo start --tunnel --clear`
